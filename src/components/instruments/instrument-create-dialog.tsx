  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (investorsResult && !investorsResult.validationErrors && !investorsResult.serverError && investorsResult.data) {
          setInvestors(investorsResult.data);
        }

        // Load rounds if companyId is provided
        if (companyId) {
          const roundsResult = await listRoundsByCompanyAction({ companyId });

          if (roundsResult && !roundsResult.validationErrors && !roundsResult.serverError && roundsResult.data) {
