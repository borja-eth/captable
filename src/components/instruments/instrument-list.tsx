...
import { listInstrumentsByRoundAction, deleteInstrumentAction } from "@/lib/actions/instrument-actions";
...
export const InstrumentList = () => {
    ...
    try {
      ...
      if (result && !result.validationErrors && !result.serverError) {
        toast({
          ...
          description: "The instrument has been deleted successfully.",
          variant: "success",
        });
        router.refresh();
      }
  ...
  const loadInstruments = async () => {
    ...
    try {
      const result = await listInstrumentsByRoundAction({ roundId: selectedRoundId });
    ...
    <>
      ...
      {selectedRoundId ? (
          ...
          <TableBody>
            {instruments.map((instrument) => (
              <TableRow key={instrument.id}>
                <TableCell className="font-medium">{instrument.type}</TableCell>
                <TableCell>{instrument.investorId}</TableCell>
                <TableCell>${instrument.amount.toLocaleString()}</TableCell>
                <TableCell>
                  {instrument.type === "EQUITY" && (
                    <>
                      {instrument.shareCount} shares at ${instrument.sharePrice}/share ({instrument.shareClass})
                    </>
                  )}
                  {instrument.type === "SAFE" && (
                    <>
                      Cap: ${instrument.valuationCap?.toLocaleString()}, Discount: {instrument.discountRate}%
                    </>
                  )}
                  {instrument.type === "CONVERTIBLE_NOTE" && (
                    <>
                      Interest: {instrument.interestRate}%, Cap: ${instrument.conversionCap?.toLocaleString()}, Discount: {instrument.discountRate}%
                    </>
                  )}
                </TableCell>
                <TableCell>
        ...
        <div className="text-center py-8 text-muted-foreground">
          {companyId ? "Please select a round to view its instruments" : "Please select a company to view its instruments"}
        </div>
      )}
